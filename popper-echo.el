...
quickly."
  (lambda (&optional arg)
    ...
    (when popper-echo-dispatch-persist (funcall repeat))))
...
Each command in the keymap calls the function REPEAT afterwards."
  (set-transient-map
   (cl-loop with map = (make-sparse-keymap)
            ...
            for rawkey = (cond ((characterp keybind) (make-vector 1 keybind))
                               ((stringp keybind) (kbd keybind)))
            do
            (define-key map rawkey (popper-echo--dispatch-toggle i buffers repeat))
...
(defun popper-tab-line--format (tab tabs)
  (let ((name (tab-line-tab-name-format-default tab tabs))
    ...
    (concat (char-to-string (+ idx #x2460))  ;; #x2776
            name)))
...
`popper-mode'."
  ...
  (if popper-tab-line-mode
    ...
    (mapc
     ...
     (mapcan #'cdr popper-buried-popup-alist))))
